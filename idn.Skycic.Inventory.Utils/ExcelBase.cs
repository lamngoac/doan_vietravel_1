...
namespace idn.Skycic.Inventory.Utils
{
    public class ExcelBase : IDisposable
    {
        ...
        public string GetRange()
        {
             ...
             */
            //giangcl- edit (allow get range only from start point - end point is auto determine)
            if (determinedrange == null)
            {
                string range = Range;
                ...
                else if (range.IndexOf(':') == -1 && !range.EndsWith("$"))
                {
                    //string testRange = DetermineRange();
                    string tmpRange = DetermineRangeForce();
                    int pos = tmpRange.IndexOf(':');
                    range = range + ":" + tmpRange.Substring(pos + 1);
                }
                determinedrange = "[" + range + "]";
            }
            return determinedrange;
        ...
        //giangcl
        /// <summary>
        /// bat buoc tra ve range, ko fai la ca sheet name
        /// </summary>
        /// <returns></returns>
        string DetermineRangeForce()
        {
        ...
        class RangeFinder
        {
            ...
            {
                this.eb = new ExcelImportNew(Owner.WorkBook);
                eb.Range = sheet;
                eb.UseHeaders = false;
                ...
                try
                {
                    ...
                    //giangcl - ko hieu sao bi mat 1 row cuoi cung o day
                    //DecreaseRange();
                }
                finally
                {
        ...
        public OleDbConnection CreateConnection()
        {
            ...
            string.Format("Provider=Microsoft.ACE.OLEDB.12.0;" +
            "Data Source={0};Extended Properties='Excel 12.0;HDR={1};Imex={2}'",
    ...
    /// 
    /// </summary>
    public class ExcelImportNew : ExcelBase
    {
        ...
        {
            var i = new ExcelImportNew(File, Range) { InterMixedAsText = true };
            return i.Query();
        }
        public static DataTable Select(string File, string Sql)
        {
            var i = new ExcelImportNew(File) { SQL = Sql, InterMixedAsText = true };
            return i.Query();
        }
        #endregion

        #region Constructors
        public ExcelImportNew() { }
        public ExcelImportNew(string WorkBook) : base(WorkBook) { }
        public ExcelImportNew(string WorkBook, string Range)
            : this(WorkBook)
        {
            this.Range = Range;
        }
        ...
        public DataTable Query(DataTable dt)
        {
            ...
            try
            {
                ...
                #region["20181017: set data type của column về string"]
                if (dt != null && dt.Rows.Count > 0)
                {
                    var tableCloned = dt.Clone();

                    if (dt.Columns.Count > 0)
                    {
                        for (var i = 0; i < dt.Columns.Count; i++)
                        {
                            tableCloned.Columns[i].DataType = typeof(string);
                        }
                    }
    ...
    public class ExcelExportNew
    {
        ...
        /// <returns></returns>
        private static OleDbConnection CreateConnection(String filePath)
        {
        ...
        /// <param name="filePath"></param>
        static public void Export(DataSet dataSet, string filePath)
        {
            if (dataSet == null)
                return;
            ...
            try
            {
                ...
                {
                    string tableName = dt.TableName.Replace(' ', '_');
                    string commandStr = "CREATE TABLE " + tableName + " (";
                    foreach (DataColumn dc in dt.Columns)
                    {
                        commandStr += dc.ColumnName + " char(255),";
                    }
                    commandStr = commandStr.Substring(0, commandStr.Length - 1);
                    commandStr += ")";
                    OleDbCommand cmd = new OleDbCommand(commandStr, connection);
                    cmd.ExecuteNonQuery();
                    //
                    string cmdFormat = "INSERT INTO {0} VALUES {1}";
                    string cmdString;
                    string tmpValue;
                    ...
                    foreach (DataRow dtRow in dt.Rows)
                    {
                        foreach (DataColumn dc in dt.Columns)
                        {
                            sbValue.AppendFormat("{0}{1}{0},", (char)1, dtRow[dc.ColumnName]);
                        }
                        //tmpValue = sbValue.ToString();
                        sbValue = sbValue.Replace("'", "").Replace((char)1, '\'');
                        tmpValue = sbValue.ToString(0, sbValue.Length - 1);
                        tmpValue = "(" + tmpValue + ")";
                        cmdString = String.Format(cmdFormat, tableName, tmpValue);
        ...
        /// <param name="dataSet"></param>
        /// <param name="filePath"></param>
        static public void Export<T>(DataSet dataSet, string filePath)
        {
            if (dataSet == null)
                return;
            OleDbConnection connection = CreateConnection(filePath);
